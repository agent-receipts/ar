...
Download the `obsigna_<version>_<os>_<arch>` tarball for your platform (darwin/linux, amd64/arm64) from the [releases page](https://github.com/agent-receipts/ar/releases?q=obsigna) and move `obsigna-mcp` (and the `mcp-proxy` shim, if you still rely on the old name) onto your `$PATH`.
...
`mcp-proxy` no longer holds a signing key — `obsigna-daemon` owns the Ed25519 key and is the sole writer of receipts ([ADR-0010](https://github.com/agent-receipts/ar/blob/main/docs/adr/0010-daemon-process-separation.md)). Generate the key once and start the daemon:
