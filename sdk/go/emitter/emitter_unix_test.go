...
func TestSessionID(t *testing.T) {
	// Test that SessionID returns the session ID set at construction.
	explicitSessionID := "test-session-123"
	em, err := New(
		WithSocketPath("/tmp/test.sock"),
		WithSessionID(explicitSessionID),
		WithLogger(silentLogger()),
	)
	if err != nil {
		t.Fatalf("New: %v", err)
	...
	if got != explicitSessionID {
		t.Errorf("SessionID() = %q, want %q", got, explicitSessionID)
	}
}

func TestSessionIDGeneratedWhenNotProvided(t *testing.T) {
	// Test that SessionID is generated (non-empty, looks like a UUID) when not provided.
	em, err := New(
		WithSocketPath("/tmp/test.sock"),
		WithLogger(silentLogger()),
	)
	...
	got := em.SessionID()
	if got == "" {
		t.Error("SessionID() returned empty string; expected a generated UUID")
	}

	// UUID v4 format is 8-4-4-4-12 hex chars with hyphens
	if len(got) != 36 || got[8] != '-' || got[13] != '-' || got[18] != '-' || got[23] != '-' {
		t.Errorf("SessionID() = %q; doesn't look like a UUID v4", got)
	}
}

func TestDefaultSocketPathEnvironment(t *testing.T) {
	// Save original env
	orig := os.Getenv("AGENTRECEIPTS_SOCKET")
	t.Cleanup(func() {
		if orig != "" {
			_ = os.Setenv("AGENTRECEIPTS_SOCKET", orig)
		} else {
			_ = os.Unsetenv("AGENTRECEIPTS_SOCKET")
		}
	})

	// When AGENTRECEIPTS_SOCKET is set, DefaultSocketPath returns it
	testPath := "/custom/socket/path.sock"
	_ = os.Setenv("AGENTRECEIPTS_SOCKET", testPath)
	got := DefaultSocketPath()
	if got != testPath {
		t.Errorf("DefaultSocketPath() = %q, want %q (when AGENTRECEIPTS_SOCKET set)", got, testPath)
	}
}

func TestDefaultSocketPathDarwin(t *testing.T) {
	if runtime.GOOS != "darwin" {
		t.Skip("darwin-only test")
	}

	// Save and restore environment
	origSocket := os.Getenv("AGENTRECEIPTS_SOCKET")
	origTmpDir := os.Getenv("TMPDIR")
	t.Cleanup(func() {
		if origSocket != "" {
			_ = os.Setenv("AGENTRECEIPTS_SOCKET", origSocket)
		} else {
			_ = os.Unsetenv("AGENTRECEIPTS_SOCKET")
		}
		if origTmpDir != "" {
	...
	// When TMPDIR is set, use it
	testTmpDir := "/custom/tmp"
	_ = os.Setenv("TMPDIR", testTmpDir)
	got := DefaultSocketPath()
	expected := filepath.Join(testTmpDir, "agentreceipts", "events.sock")
	if got != expected {
		t.Errorf("DefaultSocketPath() = %q, want %q (with TMPDIR=%s)", got, expected, testTmpDir)
	}

	// When TMPDIR is unset, use /tmp
	_ = os.Unsetenv("TMPDIR")
	got = DefaultSocketPath()
	expected = "/tmp/agentreceipts/events.sock"
	if got != expected {
		t.Errorf("DefaultSocketPath() = %q, want %q (default /tmp)", got, expected)
	}
}

func TestDefaultSocketPathLinux(t *testing.T) {
	...
	// When XDG_RUNTIME_DIR is set, use it
	testXDG := "/run/user/1000"
	_ = os.Setenv("XDG_RUNTIME_DIR", testXDG)
	got := DefaultSocketPath()
	expected := filepath.Join(testXDG, "agentreceipts", "events.sock")
	if got != expected {
		t.Errorf("DefaultSocketPath() = %q, want %q (with XDG_RUNTIME_DIR=%s)", got, expected, testXDG)
	}

	// When XDG_RUNTIME_DIR is unset, use /run/agentreceipts
	_ = os.Unsetenv("XDG_RUNTIME_DIR")
	got = DefaultSocketPath()
	expected = "/run/agentreceipts/events.sock"
	if got != expected {
		t.Errorf("DefaultSocketPath() = %q, want %q (default /run/agentreceipts)", got, expected)
	}
}
