...
- **HPKE parameter disclosure** ([#722](https://github.com/agent-receipts/ar/pull/722), ADR-0012, ADR-0015) — receipts now support optionally encrypting sensitive action parameters using HPKE (RFC 9180) v1 with X25519 KEM and AES-256-GCM. A `parameters_disclosure` envelope (kid, enc, ct) attaches to receipts when a forensic public key is configured and a disclosure policy permits the action type. The daemon uses a forensic X25519 public key (32 raw bytes) to encrypt parameters; the corresponding private key is kept offline by the forensic responder. The envelope `kid` is the SHA256 fingerprint of the public key (ADR-0015). Parameters remain hashed in `parameters_hash` regardless — the disclosure envelope is opaque without the private key. Encryption failures fall back to hash-only receipts (no audit gaps). See [Parameter Disclosure spec](https://agent-receipts.github.io/specification/parameter-disclosure/) for threat model and implementation details.
- **Forensic key generation and configuration** — new CLI flag `--init-forensic-key` generates a new X25519 keypair, writes the private key to `--forensic-key` (raw 32 bytes, mode 0600), and the public key to a derived path (mode 0644), then exits. The public key path is configured via `--forensic-public-key` (file path to 32 raw bytes) or `AGENTRECEIPTS_FORENSIC_PUBLIC_KEY` env var. TOML config key: `forensic_public_key`. The daemon never reads or holds the private key.
...
- **Action type routing in disclosure policy** — the `EmitterFrame` JSON-RPC payload now includes an optional `action_type` field (omitted by SDK v0.14.0 emitters). When the daemon receives an explicit action type, it uses that to resolve risk-based disclosure; when absent, it synthesizes a fallback `<channel>.<tool>` type (medium risk). This enables risk-based `"high"` disclosure to gate by actual action risk rather than tool name.
