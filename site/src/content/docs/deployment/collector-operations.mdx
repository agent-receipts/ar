...
This guide covers how to run the [reference collector](https://github.com/agent-receipts/ar/tree/main/collector) in production. It is the day-2 operations story — how the process is deployed, kept running, and observed. It does **not** duplicate the wire contract (POST `/receipts` semantics, status codes, validation scope) — that lives in [ADR-0020](https://github.com/agent-receipts/ar/blob/main/docs/adr/0020-emitter-abstraction-and-remote-receipt-delivery.md) and the [collector README](https://github.com/agent-receipts/ar/blob/main/collector/README.md). Cross-reference those for anything about the HTTP API itself.
...
The default store. A single file on local disk. No external dependencies, no server to manage, immediate durability via WAL mode.
...
**v0 ships without authentication.** This is a deliberate starting point, not an oversight. The client side — `HttpEmitter` — already supports `api-key`, `bearer`, and `mTLS` via `HttpEmitterAuth` ([ADR-0020](https://github.com/agent-receipts/ar/blob/main/docs/adr/0020-emitter-abstraction-and-remote-receipt-delivery.md)), so the authentication vocabulary exists. Server-side enforcement is tracked as future work.
...
→ 503  store is unreachable (database connection lost, disk full)
```
...
The collector emits structured JSON logs (`log/slog`) to stdout. Each record carries the standard slog `level`, `time`, and `msg` fields; receipt-handling records add `id`, and the accept path also adds `chain_id` and `sequence`. The collector does **not** emit an HTTP status field — derive status-code signals from the `msg` value (or from your proxy's access logs, see [Metrics](#metrics)). Key things to filter on for audit-relevant events:
...
|--------|---------------|
| Ingest rate (receipts/s) | Count proxy `2xx` responses, or `msg="receipt accepted"` log lines, per second |
| Conflict rate | Count proxy `409` responses, or `msg="receipt already exists, returning 409"` log lines, per second — elevated rate signals retry loops |
| Client error rate | Count proxy `400` responses, or `msg="receipt rejected: …"` log lines, per second — unexpected spikes indicate SDK misconfiguration |
| Server error rate | Count proxy `5xx` responses, or `level=ERROR` log lines (e.g. `msg="receipt insert failed"`), per second — correlate with store health |
...
- [Collector README](https://github.com/agent-receipts/ar/blob/main/collector/README.md) — wire contract, validation scope, configuration flags
- [Ephemeral Compute Deployment](/deployment/ephemeral-compute/) — adopter-side guide for SDK emitter configuration
- [CLI Commands](/reference/cli-commands/) — verifier tooling (`agent-receipts verify`)
