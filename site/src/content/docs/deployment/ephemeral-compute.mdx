...
- **Parallel tool calls must be serialised at the receipt layer.** Even when tool calls execute concurrently, receipt construction must pass through a single queue. Concurrent signing of independent receipts is not supported in v1.
