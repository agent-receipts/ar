...
Agent Receipts (this project) sits outside the enforcement tools in the table above — it is an audit trail, not a policy engine. In the daemon-backed deployment, signing keys live in `agent-receipts-daemon` and never enter the agent, proxy, or SDK process, so those components cannot forge or rewrite signed receipts. The SDKs and mcp-proxy also support a standalone in-process signing mode (the current default) where the key lives in the calling process.
