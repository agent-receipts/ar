...
>
> A related but distinct effort is **CAP-SRP** (VeritasChain) — a *refusal*-provenance spec proving an AI declined harmful content. It is adjacent to this layer, not a general agent-action audit format, so it solves a different problem than the cross-channel agent record.
