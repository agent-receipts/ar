...
There is no supported way to rotate the daemon's signing key today. Automated rotation — emitting a `key_rotated` event so that receipts signed by the retired key still verify after the handoff — is specified in [ADR-0015](https://github.com/agent-receipts/ar/blob/main/docs/adr/0015-key-rotation-byok-anchoring.md) but unimplemented: `KeySource.Rotate()` returns `ErrNotImplemented`, and there is no rotation event, external anchor, or BYOK (HSM/KMS) backend yet.

Do **not** "rotate" by deleting `signing.key` and re-running `--init`. The new key has no cryptographic link to the old one, so every receipt signed by the retired key fails `agent-receipts verify` against the new public key and the chain is split with no in-chain bridge between the two key eras. If you must replace a key, archive the old `signing.key.pub` and verify pre-rotation receipts against it separately until rotation support lands.
...
```bash
agent-receipts-daemon --print-config
```
...
For the full model — disclosure modes, the two-key separation, forensic recovery, encrypt-failure fallback, and the GDPR/crypto-shredding story — see [Parameter Disclosure](/specification/parameter-disclosure/).
