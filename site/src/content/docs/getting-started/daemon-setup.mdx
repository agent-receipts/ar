...
Start the daemon before launching any agent session. By default, emitters that cannot reach the socket **surface the failure** — `Emit` returns an error (after at most the 25 ms dial timeout plus the 100 ms write timeout) rather than dropping the event silently, so an unreachable daemon is detectable per the [emit failure contract](https://github.com/agent-receipts/ar/blob/main/docs/adr/0024-emit-failure-contract.md). SDKs offer an opt-in best-effort mode for callers that knowingly tolerate dropped events.
