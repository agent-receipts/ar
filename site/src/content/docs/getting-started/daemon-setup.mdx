...
To rotate the daemon's signing key, stop the daemon and run `--rotate` ([ADR-0015](https://github.com/agent-receipts/ar/blob/main/docs/adr/0015-key-rotation-byok-anchoring.md)):

```bash
agent-receipts-daemon --rotate
```
...
> **Verifying a rotated chain.** Verify from the **genesis** public key — the key that signed the chain's first receipt — not the freshly published `signing.key.pub` (which is now the *new* key only). The SDK chain verifiers traverse the rotation from the genesis key automatically; the archived `signing.key.pub.rotated-*` files preserve the older keys for this. Pointing `agent-receipts verify` at the genesis key for a rotated chain is a current follow-up.
...
External anchoring of rotation events (so the rotation history survives daemon-key compromise) is still pending — see ADR-0015's phasing — so `--rotate` does not yet provide the full post-compromise integrity guarantee.
