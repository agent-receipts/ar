...
**Thin emitter** — the plugin, proxy, or SDK fires an event describing the tool call. No signing. No storage. No crypto. Fire-and-forget over a local Unix socket. The agent is never blocked waiting for the audit layer. If the daemon is down, the event drops silently and the gap is visible in the chain on restart.
...
The receipts are W3C Verifiable Credentials, Ed25519-signed, hash-chained. A real one from this session:
...
The next post covers two of those in action — a live demo of the unified chain and secret redaction working together.
