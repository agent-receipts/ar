...
```json
{
  ...
  "credentialSubject": {
    "principal": {
      "id": "did:session:agent:main:telegram:direct:114070211"
    },
    "action": {
    ...
    "chain": {
      ...
      "chain_id": "chain_openclaw_agent:main:telegram:direct:114070211_52de0419-1d76-4d15-9fb4-ffe5463e5cd2"
    }
  },
  "proof": {
...
**Full parameter storage.** The proxy can store the complete, unredacted parameters (subject to secret-redaction patterns) when configured to do so. The plugin's privacy-preserving default is hash-only. Even with `parameterPreview: true`, what's stored is a best-effort plaintext representation, not the full canonical arguments object.
