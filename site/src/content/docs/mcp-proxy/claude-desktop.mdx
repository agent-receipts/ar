...
`-http 127.0.0.1:8080` pins the approval endpoint to a known port so an approver can target it. Without a running approver, paused calls fail with `-32002: approval timeout or rejected` — see [Approval Server](/mcp-proxy/approval-ui/). If you also run Claude Code or Codex simultaneously, give each client a distinct port (e.g. `8080`, `8081`, `8082`).
