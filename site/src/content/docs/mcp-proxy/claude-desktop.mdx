...
The default policy pauses calls scoring ≥50 (auth/token/secret/key/credential tools, SQL mutations without `WHERE`). Most GitHub tools are well below that — `create_pull_request` scores 20 — so you won't see pauses unless your server exposes sensitive tools. The config below pins `-http 127.0.0.1:8080` so an approver has a stable URL when you do need one. See [Approval Server](/mcp-proxy/approval-ui/).
:::
