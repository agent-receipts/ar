...
With the default policy, high-risk writes (e.g. `create_pull_request`) are paused until approved. The configs below pin `-http 127.0.0.1:8081` so an approver has a stable URL to talk to. Skip the approver and those calls fail with `-32002: approval timeout or rejected` after the approval timeout — while reads and low-risk writes (`push_files`, `create_branch`) still succeed, making the symptom look tool-specific. See [Approval Server](/mcp-proxy/approval-ui/).
:::
