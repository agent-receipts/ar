...
The default policy pauses calls scoring ≥50 (auth/token/secret/key/credential tools, SQL mutations without `WHERE`). Most GitHub tools are well below that — `create_pull_request` scores 20, `merge_pull_request` scores 10 — so you won't see pauses unless your server exposes sensitive tools. The configs below pin `-http 127.0.0.1:8081` so an approver has a stable URL when you do need one. If you're seeing `-32002` on tools that *shouldn't* pause, suspect client-side denial first — see [troubleshooting](/mcp-proxy/configuration/#-32002-approval-timeout-or-rejected).
:::
