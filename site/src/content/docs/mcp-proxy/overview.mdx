...
The built-in policy contains a `pause_high_risk` rule (`min_risk_score: 50`). Most tools don't reach it: ordinary GitHub writes like `create_pull_request` (risk 20) and `merge_pull_request` (risk 10) pass straight through. But calls scoring ≥50 — tool names containing `auth`/`token`/`secret`/`key`/`credential`/`password`, `*config*`/`*setting*` writes, SQL mutations without a `WHERE` clause — are held until something POSTs an approve/deny decision. With no approver, any such call fails with `-32002: approval timeout or rejected` after the approval timeout.

If your MCP server exposes tools that can hit the threshold, either pin `-http 127.0.0.1:8081` and run an approver (see [Approval Server](/mcp-proxy/approval-ui/)), or load a custom `-rules` YAML with no `pause`/`block` rules. Seeing `-32002` on tools that *shouldn't* pause (like `create_pull_request`) is usually client-side denial, not the proxy — see [troubleshooting](/mcp-proxy/configuration/#-32002-approval-timeout-or-rejected).
:::
