...
### `-32002: approval timeout or rejected`
...
The remedy is to run an approver, or to relax the policy. See [Approval Server](/mcp-proxy/approval-ui/) for both paths. Common tools that *do* cross the default threshold of 50: names containing `auth`/`token`/`secret`/`key`/`credential`/`password`, `*config*`/`*setting*` tools combined with writes, SQL mutations without `WHERE`. Ordinary GitHub writes like `create_pull_request` (risk 20) and `merge_pull_request` (risk 10) don't pause under the default scorer — if those are failing, you're in path #1, not #2.
