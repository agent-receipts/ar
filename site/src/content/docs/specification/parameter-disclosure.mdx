...
The `parameter_disclosure` config key controls which actions get an encrypted envelope attached. The value space is the same across the daemon config and the OpenClaw plugin:

| Value | Behavior |
|-------|----------|
| `false` | Hash only — no envelope (default) |
| `true` | Envelope for every action |
| `"high"` | Envelope for `high` and `critical` risk actions only |
| `["system.command.execute", "filesystem.file.delete"]` | Envelope for the listed action types only |

The `"high"` mode uses the taxonomy's risk classification. The array form is an explicit allowlist of action-type strings; see the [Action Taxonomy](/specification/action-taxonomy/) for the full list.
...
# Disclosure mode: false | true | "high" | ["action.type", ...]
...
The daemon will not start with `parameter_disclosure != false` and no `forensic_public_key` configured — an envelope requires a recipient.
...
See [Daemon Setup](/getting-started/daemon-setup/) for the full configuration reference.
...
1. Generate a forensic keypair (X25519) with `agent-receipts-daemon --init-forensic-key` (see [Generate a forensic key pair](#generate-a-forensic-key-pair) above).
