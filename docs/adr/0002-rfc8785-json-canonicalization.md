...
- **Good library support across target languages** — the algorithm is straightforward enough that all three SDKs (Go, Python, TypeScript) implement it directly without external dependencies, keeping the dependency tree small and auditable. The TypeScript implementation benefits from native ES6 number serialization; Go and Python re-implement the ES6 `Number.toString()` algorithm.
...
This risk is the same canonicalization dependency identified in ADR-0001's Security Considerations. Mitigation requires:
